...
export class Web3AuthController {
  ...
  static async getChallenge(req: Request, res: Response) {
    try {
      ...
      const challenge = await Web3AuthService.generateChallenge(address);
  ...
  static async verifySignature(req: Request, res: Response) {
    try {
      ...
      const result = await Web3AuthService.verifySignature(
        address,
        signature,
        message
      );
  ...
  static async refreshToken(req: Request, res: Response) {
    try {
      ...
      const result = await Web3AuthService.refreshToken(token);
  ...
  static async getCurrentUser(req: Request, res: Response) {
    try {
      ...
      if (req.user.address) {
        const userInfo = await Web3AuthService.getUserByWallet(
          req.user.address
        );
        return res.json({
          success: true,
